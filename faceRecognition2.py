...
def get_cropimg(now_id, frame_id, x1, y1, w, h, firstFrame, model="Dlib"):  # default model: dlib
    ...
    if model == "Dlib":
        ...
        img_encoding = get_dlib_encoding(found_rect, frame)

        if firstFrame == True:
            face_vectors.append([img_encoding, now_id])
            return crop_face, now_id

        found_id, valid = find_by_euclidean(img_encoding, now_id)
    ...
    if model == "FaceNet":
        ...
        found_id = check_existence(now_id, encode, firstFrame)

    return crop_face, found_id


def get_dlib_encoding(found_rect, frame):
...
def find_by_euclidean(img, now_id):
    valid = True
    distance = float("inf")
    found_id = now_id
    check_dis = 0
...
def check_existence(now_id, encode, firstFrame):
    distance = float("inf")
...
def set_vid_name_for_recognition(name_):
    global vid
    vid = name_
    global cap
