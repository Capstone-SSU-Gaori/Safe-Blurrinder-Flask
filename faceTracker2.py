...
import cv2
from face_recognition import set_vid_name_for_recognition, get_cropimg
import dlib
...
def start_tracker(video_name):
    global vid_name
    vid_name = video_name
    if vid_name is None:
        print("video name needs to be set")
        return

    cap, out = set_video_settings()
    set_vid_name_for_recognition(vid_name)  # 얼굴인식 파일에서 사용할 비디오 이름 setting
        ...
        if i == 0:  # 첫 번째 프레임이라면?
            ...
            if result != []:  # 감지된 얼굴이 존재한다면
                for person in result:
                    ...
                    crop, found_id = get_cropimg(id_, i, x, y, w, h, True, model_name)
            ...
            if (len(this_box) > len(trackers)):
                ...
                for t in range(len(this_box)):
                    ...
                    crop, found_id = get_cropimg(id_, i, x, y, w, h, False, model_name)
